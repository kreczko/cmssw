#include "DQMOffline/L1Trigger/interface/L1TJetMetAnalyzer_Offline.h"
...
// -------------------------------------- Constructor --------------------------------------------
//
L1TJetMetAnalyzer_Offline::L1TJetMetAnalyzer_Offline(const edm::ParameterSet& ps) :
	...
	edm::LogInfo("L1TJetMetAnalyzer_Offline") << "Constructor "
			<< "L1TJetMetAnalyzer_Offline::L1TJetMetAnalyzer_Offline " << std::endl;
}

//
// -- Destructor
//
L1TJetMetAnalyzer_Offline::~L1TJetMetAnalyzer_Offline() {
	edm::LogInfo("L1TJetMetAnalyzer_Offline") << "Destructor L1TJetMetAnalyzer_Offline::~L1TJetMetAnalyzer_Offline "
			<< std::endl;
}
...
// -------------------------------------- beginRun --------------------------------------------
//
void L1TJetMetAnalyzer_Offline::dqmBeginRun(edm::Run const &, edm::EventSetup const &) {
	edm::LogInfo("L1TJetMetAnalyzer_Offline") << "L1TJetMetAnalyzer_Offline::beginRun" << std::endl;
}
//
// -------------------------------------- bookHistos --------------------------------------------
//
void L1TJetMetAnalyzer_Offline::bookHistograms(DQMStore::IBooker & ibooker_, edm::Run const &,
		edm::EventSetup const &) {
	edm::LogInfo("L1TJetMetAnalyzer_Offline") << "L1TJetMetAnalyzer_Offline::bookHistograms" << std::endl;
...
//
void L1TJetMetAnalyzer_Offline::beginLuminosityBlock(edm::LuminosityBlock const& lumiSeg,
		edm::EventSetup const& context) {
	edm::LogInfo("L1TJetMetAnalyzer_Offline") << "L1TJetMetAnalyzer_Offline::beginLuminosityBlock" << std::endl;
}

//
// -------------------------------------- Analyze --------------------------------------------
//
void L1TJetMetAnalyzer_Offline::analyze(edm::Event const& e, edm::EventSetup const& eSetup) {
	edm::LogInfo("L1TJetMetAnalyzer_Offline") << "L1TJetMetAnalyzer_Offline::analyze" << std::endl;

	edm::Handle<reco::VertexCollection> vertexHandle;
	e.getByToken(thePVCollection_, vertexHandle);
	if (!vertexHandle.isValid()) {
		edm::LogError("L1TJetMetAnalyzer_Offline") << "invalid collection: vertex " << std::endl;
		return;
	}
...
void L1TJetMetAnalyzer_Offline::fillEnergySums(edm::Event const& e, unsigned int nVertex) {
	...
	if (!caloJets.isValid()) {
		edm::LogError("L1TJetMetAnalyzer_Offline") << "invalid collection: calo jets " << std::endl;
		return;
	}
	if (!caloMETs.isValid()) {
		edm::LogError("L1TJetMetAnalyzer_Offline") << "invalid collection: calo MET " << std::endl;
		return;
	}
	if (!l1EtSums.isValid()) {
		edm::LogError("L1TJetMetAnalyzer_Offline") << "invalid collection: L1 ET sums " << std::endl;
		return;
	}
...
void L1TJetMetAnalyzer_Offline::fillJets(edm::Event const& e, unsigned int nVertex) {
	...
	if (!l1Jets.isValid()) {
		edm::LogError("L1TJetMetAnalyzer_Offline") << "invalid collection: L1 jets " << std::endl;
		return;
	}
...
//	}
	...
	if (!foundMatch) {
		edm::LogError("L1TJetMetAnalyzer_Offline") << "Could not find a matching L1 jet " << std::endl;
		return;
	}
	...
	if (abs(recoEta) <= 1.479) { // barrel
		...
		// turn-ons
		h_turnOnJet_HB_total_->Fill(recoEt);
		h_turnOnJet_HB_HE_total_->Fill(recoEt);
		if (l1Et > minEtForTurnOns) {
			h_turnOnJet_HB_pass_->Fill(recoEt);
			h_turnOnJet_HB_HE_pass_->Fill(recoEt);
		}
		...
		// turn-ons
		h_turnOnJet_HE_total_->Fill(recoEt);
		h_turnOnJet_HB_HE_total_->Fill(recoEt);
		if (l1Et > minEtForTurnOns) {
			h_turnOnJet_HE_pass_->Fill(recoEt);
			h_turnOnJet_HB_HE_pass_->Fill(recoEt);
		}
	} else { // forward jets
		// et
		...
		// turn-ons
		h_turnOnJet_HF_total_->Fill(recoEt);
		if (l1Et > minEtForTurnOns) {
			h_turnOnJet_HF_pass_->Fill(recoEt);
		}
	}
}
...
// -------------------------------------- endLuminosityBlock --------------------------------------------
//
void L1TJetMetAnalyzer_Offline::endLuminosityBlock(edm::LuminosityBlock const& lumiSeg, edm::EventSetup const& eSetup) {
	edm::LogInfo("L1TJetMetAnalyzer_Offline") << "L1TJetMetAnalyzer_Offline::endLuminosityBlock" << std::endl;
}

//
// -------------------------------------- endRun --------------------------------------------
//
void L1TJetMetAnalyzer_Offline::endRun(edm::Run const& run, edm::EventSetup const& eSetup) {
	edm::LogInfo("L1TJetMetAnalyzer_Offline") << "L1TJetMetAnalyzer_Offline::endRun" << std::endl;
}

//
// -------------------------------------- book histograms --------------------------------------------
//
void L1TJetMetAnalyzer_Offline::bookHistos(DQMStore::IBooker & ibooker) {
	bookEnergySumHistos(ibooker);
	bookJetHistos(ibooker);
}

void L1TJetMetAnalyzer_Offline::bookEnergySumHistos(DQMStore::IBooker & ibooker) {
	ibooker.cd();
	ibooker.setCurrentFolder("L1T/JetMet");
	...
	// energy sum turn ons
	h_turnOnMET_pass_ = ibooker.book1D("turnOnMET_Num", "MET turn-on; calo MET; events", 50, -0.5, 499.5);
	h_turnOnMHT_pass_ = ibooker.book1D("turnOnMHT_Num", "MHT turn-on; reco MHT; events", 50, -0.5, 499.5);
	h_turnOnETT_pass_ = ibooker.book1D("turnOnETT_Num", "ETT turn-on; calo ETT; events", 50, -0.5, 499.5);
	h_turnOnHTT_pass_ = ibooker.book1D("turnOnHTT_Num", "HTT turn-on; reco HTT; events", 50, -0.5, 499.5);

	h_turnOnMET_total_ = ibooker.book1D("turnOnMET_Den", "MET turn-on; calo MET; events", 50, -0.5, 499.5);
	h_turnOnMHT_total_ = ibooker.book1D("turnOnMHT_Den", "MHT turn-on; reco MHT; events", 50, -0.5, 499.5);
	h_turnOnETT_total_ = ibooker.book1D("turnOnETT_Den", "ETT turn-on; calo ETT; events", 50, -0.5, 499.5);
	h_turnOnHTT_total_ = ibooker.book1D("turnOnHTT_Den", "HTT turn-on; reco HTT; events", 50, -0.5, 499.5);

	ibooker.cd();
}

void L1TJetMetAnalyzer_Offline::bookJetHistos(DQMStore::IBooker & ibooker) {
	ibooker.cd();
	ibooker.setCurrentFolder("L1T/JetMet");
	// jet reco vs L1
	...
	h_L1JetEtavsCaloJetEta_ = ibooker.book2D("L1JetETvsCaloJetET_HB",
	...
	// jet turn-ons
	h_turnOnJet_HB_pass_ = ibooker.book1D("turnOnJet_HB_Num", "jet turn-on (HB); reco jet ET; events", 300, 0, 300);
	h_turnOnJet_HE_pass_ = ibooker.book1D("turnOnJet_HE_Num", "jet turn-on (HE); reco jet ET; events", 300, 0, 300);
	h_turnOnJet_HF_pass_ = ibooker.book1D("turnOnJet_HF_Num", "jet turn-on (HF); reco jet ET; events", 300, 0, 300);
	h_turnOnJet_HB_HE_pass_ = ibooker.book1D("turnOnJet_HB_HE_Num", "jet turn-on (HB+HE); reco jet ET; events", 300, 0,
			300);

	h_turnOnJet_HB_total_ = ibooker.book1D("turnOnJet_HB_Den", "jet turn-on (HB); reco jet ET; events", 300, 0, 300);
	h_turnOnJet_HE_total_ = ibooker.book1D("turnOnJet_HE_Den", "jet turn-on (HE); reco jet ET; events", 300, 0, 300);
	h_turnOnJet_HF_total_ = ibooker.book1D("turnOnJet_HF_Den", "jet turn-on (HF); reco jet ET; events", 300, 0, 300);
	h_turnOnJet_HB_HE_total_ = ibooker.book1D("turnOnJet_HB_HE_Den", "jet turn-on (HB+HE); reco jet ET; events", 300,
			0, 300);
...
// -------------------------------------- functions --------------------------------------------
//
double L1TJetMetAnalyzer_Offline::Distance(const reco::Candidate & c1, const reco::Candidate & c2) {
	return deltaR(c1, c2);
}

double L1TJetMetAnalyzer_Offline::DistancePhi(const reco::Candidate & c1, const reco::Candidate & c2) {
	return deltaPhi(c1.p4().phi(), c2.p4().phi());
}

// This always returns only a positive deltaPhi
double L1TJetMetAnalyzer_Offline::calcDeltaPhi(double phi1, double phi2) {
...
DEFINE_FWK_MODULE (L1TJetMetAnalyzer_Offline);
